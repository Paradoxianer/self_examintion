...
class TimeChartWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (assessmentHistory.isEmpty) return const Center(child: Text("No data available"));
  ...
  List<FlSpot> getOverallScores(List<AssessmentEntry> history) {
    return history.map((entry) {
      ...
      for (var val in entry.values) { if (val != -1.0) { avg += val; count++; } }
      return FlSpot(entry.timestamp.millisecondsSinceEpoch.toDouble(), count > 0 ? avg / count : 0);
    }).toList();
  }
  ...
  List<FlSpot> getQuestionScores(List<AssessmentEntry> history, int index) {
    ...
    for (var entry in history) {
      if (index < entry.values.length && entry.values[index] != -1.0) {
        spots.add(FlSpot(entry.timestamp.millisecondsSinceEpoch.toDouble(), entry.values[index]));
      }
    }
    return spots;
  }
